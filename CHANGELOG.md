...
- **fix(validate): accept both `"0.5"` and `"0.6"` during #533 transition** (#533, #561): Added `ACCEPTED_VBRIEF_VERSIONS = frozenset({"0.5", "0.6"})` to both `scripts/vbrief_validate.py` and `scripts/spec_validate.py` so mixed pre-migration (`0.5`) and post-migration (`0.6`) trees validate cleanly while Agent 2's #533 schema vendor PR lands. Both validators carry a `TODO(#533): tighten to 0.6-only after Agent 2 merge` comment. Error messages report the full accepted set. No other validation rules changed.
