...
### Fixed
- **fix(skills): interview click-commit and always-structured rendering** (#477, #478, #506): Closed RC3 rendering gaps discovered during rc3-test1 `deft-directive-setup` Phase 2 on Warp. Extended `skills/deft-directive-interview/SKILL.md` -- (1) Rule 2 gains Always-Structured Rendering (Option A) making EVERY user-facing question a structured-tool call (click-commit where the host has one, plain-text with typed response otherwise); freeform collection becomes an explicit two-step flow; only the Rule 6 Gate and non-question status updates may be plain-text. (2) Rule 2 gains Click-Commit Rendering -- `Back` clickable on every question except the first, `Discuss with agent` clickable on every question, answer options render with the `[default]` marker inline, MUST-NOTs for omitting either affordance and for treating a click-commit return as Rule-8-compliant. (3) Rule 2 gains Preamble Placement -- preamble MAY appear as plain markdown above the structured-tool call, but the question and its enumerable options MUST live in the tool's question/options fields. (4) Rule 6 gains a Click-Commit Hosts Plain-Text Confirmation Gate subsection requiring a typed `yes`/`no`/`back`/`discuss` response and forbidding rendering the Gate via a click-commit tool. (5) Rule 6 gains Mode Restore After the Gate -- plain-text mode is RELEASED after the typed commit; the next user-facing question returns to structured rendering. (6) Rule 11 rewritten with two-mode legend behavior: plain-text mode renders the `Enter confirm / b back / 0 discuss` keystroke legend under every question; click-commit mode renders `Back`/`Discuss with agent` as clickable options and the legend MAY be omitted. Edited `skills/deft-directive-setup/SKILL.md` phase-transition prompts (Phase 1->2, Phase 2->3, Phase 3->build handoff) -- rewritten from `~ Ask if user wants to continue to Phase X` to `! Emit a structured-tool question ...` with `Yes (continue)` / `Not now (exit setup)` / `Discuss` / `Back (revisit previous phase)` options. Per #506 D1 and the #478 review comment, the `rendering_policy` skill frontmatter flag is explicitly NOT added -- that is reserved for #476 (deferred). New content tests: `tests/content/test_interview_click_commit.py` (covers #477 A/B/C) and `tests/content/test_interview_always_structured.py` (covers #478 Fixes 1/2/3/4/6 plus setup phase-transition rewrites). Existing `tests/content/test_interview_deterministic.py` continues to pass (Rules 8-11 examples and legend preserved).
